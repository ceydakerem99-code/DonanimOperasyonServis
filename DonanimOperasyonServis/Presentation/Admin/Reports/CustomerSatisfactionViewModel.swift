...
@Observable
@MainActor
final class CustomerSatisfactionViewModel {
...
#if DEBUG
extension CustomerSatisfactionViewModel {
    static func previewReady() -> CustomerSatisfactionViewModel {
        let vm = CustomerSatisfactionViewModel(
            actor: AdminPreviewData.adminUser,
            dependencies: DIContainer.mock().makeAdminDependencies()
        )
        vm.phase = .ready
        vm.summary = CustomerSatisfactionSummary(
            averageRating: 4.2,
            submittedCount: 2,
            pendingCount: 1,
            expiredCount: 0,
            totalCount: 3,
            ratingDistribution: [
                .one: 0, .two: 0, .three: 0, .four: 1, .five: 1
            ]
        )
        vm.ratingBars = CustomerSatisfactionAggregator.ratingBars(from: vm.summary!)
        return vm
    }
}
#endif
