...
struct FirebaseNotificationRepository: NotificationRepository {
    ...
    func list(for recipientId: UserID, unreadOnly: Bool) async throws -> [AppNotification] {
        try await FirebaseRepositoryMapper.run(entity: "AppNotification", id: recipientId.rawValue) {
    ...
    func save(_ notification: AppNotification) async throws {
        try await FirebaseRepositoryMapper.run(entity: "AppNotification", id: notification.id.rawValue) {
    ...
    func markAsRead(id: NotificationID) async throws {
        try await FirebaseRepositoryMapper.run(entity: "AppNotification", id: id.rawValue) {
